...
如果你使用第三方服务器项目进行公网部署，请注意以下安全事项：

1. 服务端的 WebSocket 和 HTTP 端口不要直接暴露公网，请使用防火墙（如 iptables）限制访问来源
2. `config.yaml` 中的 `auth.enabled` 务必设为 `true`，并配置设备 MAC 白名单
3. TTS、LLM 等第三方 API Key 不要明文写在配置文件中，端口一旦被扫到，配置内容会全部泄露
4. 小智具备摄像头调用能力，端口裸奔等于将摄像头开放给任何人
