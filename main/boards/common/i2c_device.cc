...
void I2cDevice::WriteReg(uint8_t reg, uint8_t value) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "WriteReg(0x%02X) failed: %s", reg, esp_err_to_name(ret));
    }
}
...
uint8_t I2cDevice::ReadReg(uint8_t reg) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "ReadReg(0x%02X) failed: %s", reg, esp_err_to_name(ret));
    }
    return buffer[0];
}
...
void I2cDevice::ReadRegs(uint8_t reg, uint8_t* buffer, size_t length) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "ReadRegs(0x%02X, len=%d) failed: %s", reg, (int)length, esp_err_to_name(ret));
    }
}